...
import com.ok.mathlightalarm.database.AlarmContract.Alarm;
...
public class AlarmDBHelper extends SQLiteOpenHelper {
	...
	private static final String SQL_CREATE_ALARM = "CREATE TABLE " + Alarm.TABLE_NAME + " (" +
			...
			Alarm.COLUMN_NAME_MATH_ENABLED + " INTEGER," +
			Alarm.COLUMN_NAME_ALARM_FLASH_ENABLED + " BOOLEAN" +
	    " )";
	...
	private AlarmModel populateModel(Cursor c) {
		...
		model.isMathEnabled = c.getInt(c.getColumnIndex(Alarm.COLUMN_NAME_MATH_ENABLED));
		model.isFlashEnabled= c.getInt(c.getColumnIndex(Alarm.COLUMN_NAME_ALARM_FLASH_ENABLED)) == 0 ? false : true;
	...
	private ContentValues populateContent(AlarmModel model) {
        ...
        values.put(Alarm.COLUMN_NAME_MATH_ENABLED, model.isMathEnabled);
        values.put(Alarm.COLUMN_NAME_ALARM_FLASH_ENABLED, model.isFlashEnabled);
